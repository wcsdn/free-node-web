...
async function main() {
  console.log('Testing /api/game/user-info (should auto-create city)...');
  const r = await testEndpoint('/api/game/user-info', 'GET');
  console.log('Status:', r.status);
  try {
    const json = JSON.parse(r.data);
    console.log('CityList:', JSON.stringify(json.data?.CityList, null, 2));
  } catch (e) {
