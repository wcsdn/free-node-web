...
import technicsData from '../config/technics.json';
...
// taskFlag: 0=普通部队, 1=驻防部队
// arrayType[0]: 兵种类型，12=盟军享受95%时间优惠
// 【重要】必须放在 /:id 之前
// ============================================
app.get('/move-time', async (c) => {
  ...
  const arrayType = arrayTypeStr.split(',').map(Number);

  if (!cityPos || !otherCityPos) {
    return error(c, 'city_pos and target_pos are required');
  }

  try {
    // C# 配置常量
    const BIG_MAP_LENGTH = 20; // int.Parse(ConfigurationManager.AppSettings["BigMapLength"])
    const DISTANCE_TIME_RATE = 18; // int.Parse(ConfigurationManager.AppSettings["DistanceTimeRate"])
    const FIXED_MOVE_TIME = 300; // int.Parse(ConfigurationManager.AppSettings["FixedMoveTime"])
    const MOVE_TO_ARENA_TIME = 30; // int.Parse(ConfigurationManager.AppSettings["MoveToArenaTime"]) - 竞技场移动时间(秒)
    const EVENT_TIME_PERCENT = 100; // int.Parse(ConfigurationManager.AppSettings["EventTimePercent"])
    ...
    if (taskFlag === 0) {
      // 普通部队移动
      // distance = sqrt((x1-x2)^2 + (y1-y2)^2) * 18
      ...
      seconds = Math.round(
        (DISTANCE_TIME_RATE * distance + FIXED_MOVE_TIME) *
        EVENT_TIME_PERCENT * 0.01
      );

      // 同盟友军 95% 时间 (arrayType[0] == 12)
      // C#: if (arrayType[0] == 12) { seconds = Convert.ToInt32(seconds * 0.95f); }
      if (arrayType[0] === 12) {
      ...
      seconds = Math.round(
        (DISTANCE_TIME_RATE * distance + 300) *
        EVENT_TIME_PERCENT * 0.01
      );
    }

    // ============================================
    // 【科技加速】Technic[15] - 部队移动速度提升
    // C#: if (XmlData.Technic.ContainsKey(15) && XmlData.Technic[15].InteriorData.ContainsKey(cityInter.TechnicLevel[14]))
    //        seconds = Convert.ToInt32(seconds * (XmlData.Technic[15].InteriorData[cityInter.TechnicLevel[14]].EffValue * 0.01f));
    // ============================================
    // 科技加速从 technics 表读取，配置从 technics.json 读取
    const TECHNIC_SPEEDUP_ID = 15; // 移动速度科技
    const techSpeedup = await db.prepare(`
      SELECT technic_level FROM technics
      WHERE wallet_address = ? AND static_index = ? AND technic_level > 0
    `).bind(walletAddress, TECHNIC_SPEEDUP_ID).first();

    if (techSpeedup) {
      // 科技效果从配置获取
      const technicsConfig = (technicsData as any).Technic || [];
      const techCfg = technicsConfig.find((t: any) => t.ID === TECHNIC_SPEEDUP_ID);
      if (techCfg && techCfg.InteriorData) {
        const levelKey = `Level${(techSpeedup as any).technic_level}`;
        const effValue = parseFloat(techCfg.InteriorData[levelKey] || techCfg.InteriorData['Level1'] || '0') * 0.01;
        seconds = Math.round(seconds * effValue);
      }
    }

    // 竞技场位置特殊处理
    // C#: if (MapUnit.IsArenaPos(otherCityPos) && FestivalActive.IsAtArenaTime()) seconds = MoveToArenaTime;
    // 竞技场位置判断：位置 % BIG_MAP_LENGTH == 0 且 > 300
    const isArenaPos = (otherCityPos % BIG_MAP_LENGTH === 0) && (otherCityPos / BIG_MAP_LENGTH > 15);
    if (isArenaPos) {
      // TODO: FestivalActive.IsAtArenaTime() 需要节日活动时间检查
      // 暂时用配置时间，简化处理
    ...
    return success(c, {
      city_pos: cityPos,
      target_pos: otherCityPos,
      task_flag: taskFlag,
      array_type: arrayType,
      distance,
      seconds,
      message: `Movement time: ${formatDuration(seconds)}`,
    });
  } catch (err: any) {
    console.error('GetMoveTime error:', err);
    return error(c, err.message);
  }
});
