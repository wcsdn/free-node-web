...
export const heroService = {
  /** 获取武将列表 */
  async getList(db: D1Database, walletAddress: string): ServiceResult<any[]> {
    try {
  ...
  /** 获取武将详情 */
  async getDetail(db: D1Database, heroId: number): ServiceResult<any> {
    try {
  ...
  /** 招募武将 */
  async recruit(db: D1Database, walletAddress: string, cityId: number, name: string): ServiceResult<any> {
    try {
      const hero = await heroRepo.create(db, { city_id: cityId, wallet_address: walletAddress, name });
      return { ok: true, data: hero ? { id: hero.id, name: hero.name, quality: hero.quality } : null };
    } catch (error) {
      return { ok: false, error: (error as Error).message, status: 500 };
    }
  },
  ...
  /** 升级武将 */
  async levelUp(db: D1Database, heroId: number): ServiceResult<any> {
    try {
  ...
  /** 设置技能 */
  async setSkill(db: D1Database, heroId: number, skill: string): ServiceResult<never> {
    try {
