  ...
  return (
    <div className={gufengStyles.container}>
      <div className={gufengStyles.header}>
        <h2>帮派</h2>
        {myUnion && <span className={gufengStyles.unionName}>{myUnion.name}</span>}
      ...
      <div className={gufengStyles.typeNav}>
        {typeNames.map((name, idx) => (
          <button
            key={idx}
            className={`${gufengStyles.typeBtn} ${currentType === idx ? gufengStyles.active : ''}`}
      ...
      <div className={gufengStyles.content}>
        {/* 加载状态 */}
        {loading && (
          <div className={gufengStyles.loadingOverlay}>
            <div className={gufengStyles.loadingText}>正在加载...</div>
          </div>
        )}

        {/* 我的帮派 */}
        {currentType === 0 && (
          <div className={gufengStyles.myUnionPanel}>
            {myUnion ? (
              <>
                <div className={gufengStyles.unionInfo}>
                  <div className={gufengStyles.unionHeader}>
                    <div className={gufengStyles.unionIcon}>帮</div>
                    <div className={gufengStyles.unionBasic}>
                      <h3>{myUnion.name}</h3>
                      <p>等级 {myUnion.level} · {myUnion.memberCount}/{myUnion.maxMembers}人</p>
                    </div>
                  </div>
                  <div className={gufengStyles.unionStats}>
                    <div className={gufengStyles.statItem}>
                      <span className={gufengStyles.statLabel}>名望</span>
                      <span className={gufengStyles.statValue}>{myUnion.fame}</span>
                    </div>
                    <div className={gufengStyles.statItem}>
                      <span className={gufengStyles.statLabel}>声望</span>
                      <span className={gufengStyles.statValue}>{myUnion.prestige}</span>
                    </div>
                  </div>
                  <div className={gufengStyles.unionNotice}>
                    <h4>帮派公告</h4>
                    <p>{myUnion.notice || '暂无公告'}</p>
                  </div>
                </div>
                <div className={gufengStyles.unionActions}>
              ...
              <div className={gufengStyles.noUnion}>
                <p>您还没有加入帮派</p>
                <button className={gufengStyles.createBtn} onClick={() => setShowCreateModal(true)}>创建帮派</button>
                <button className={gufengStyles.joinBtn} onClick={() => setCurrentType(3)}>加入帮派</button>
              </div>
            )}
          </div>
        )}
        ...
        {currentType === 1 && myUnion && (
          <div className={gufengStyles.memberListPanel}>
            <div className={gufengStyles.memberTitle}>
              <span>职位</span>
              <span>成员</span>
              <span>等级</span>
              <span>贡献</span>
            ...
            <div className={gufengStyles.memberList}>
              {members.map((member) => (
                <div key={member.id} className={gufengStyles.memberItem}>
                  <span className={gufengStyles.memberJob}>{jobNames[member.job]}</span>
                  <span className={gufengStyles.memberName}>
                    {member.name}
                    {member.online && <span className={gufengStyles.onlineDot}></span>}
                  </span>
                  <span className={gufengStyles.memberLevel}>{member.level}</span>
                  <span className={gufengStyles.memberContribution}>{member.contribution}</span>
                  <span className={gufengStyles.memberStatus}>{member.online ? '在线' : member.lastActive}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {currentType === 2 && myUnion && (
          <div className={gufengStyles.applicantPanel}>
            <div className={gufengStyles.applicantTitle}>待审批申请 ({applicants.length})</div>
            <div className={gufengStyles.applicantList}>
              {applicants.length === 0 ? (
                <div className={gufengStyles.empty}>暂无申请</div>
              ) : (
                applicants.map((applicant) => (
                  <div key={applicant.id} className={gufengStyles.applicantItem}>
                    <div className={gufengStyles.applicantInfo}>
                      <span className={gufengStyles.applicantName}>{applicant.name}</span>
                      <span className={gufengStyles.applicantLevel}>等级 {applicant.level}</span>
                    </div>
                    <div className={gufengStyles.applicantActions}>
        ...
        {/* 帮派列表 */}
        {currentType === 3 && (
          <div className={gufengStyles.unionListPanel}>
            <div className={gufengStyles.unionListTitle}>可申请帮派</div>
            <div className={gufengStyles.unionList}>
              {unionList.map((union) => (
                <div key={union.id} className={gufengStyles.unionItem}>
                  <div className={gufengStyles.unionItemInfo}>
                    <span className={gufengStyles.unionItemName}>{union.name}</span>
                    <span className={gufengStyles.unionItemLeader}>帮主: {union.leader}</span>
                    <span className={gufengStyles.unionItemSize}>{union.memberCount}/{union.maxMembers}人</span>
                  </div>
                  <button onClick={() => handleJoin(union.id)}>申请</button>
                </div>
              ))}
            </div>
            <div className={gufengStyles.pagination}>
        ...
        {/* 帮派聊天 */}
        {currentType === 4 && (
          <div className={gufengStyles.chatPanel}>
            <div className={gufengStyles.chatMessages}>
              <div className={gufengStyles.chatMessage}>
                <span className={gufengStyles.msgSender}>帮主A</span>
                <span className={gufengStyles.msgContent}>欢迎大家加入帮派！</span>
              </div>
              <div className={gufengStyles.chatMessage}>
                <span className={gufengStyles.msgSender}>副帮主B</span>
                <span className={gufengStyles.msgContent}>今晚8点帮战，大家准时参加！</span>
              </div>
            </div>
            <div className={gufengStyles.chatInput}>
        ...
        {/* 帮派物资 */}
        {currentType === 5 && (
          <div className={gufengStyles.resourcePanel}>
            <div className={gufengStyles.resourceTitle}>帮派物资</div>
            <div className={gufengStyles.resourceList}>
            ...
            <div className={gufengStyles.resourceActions}>
              <button>捐献物资</button>
              <button>申请物资</button>
            </div>
          </div>
