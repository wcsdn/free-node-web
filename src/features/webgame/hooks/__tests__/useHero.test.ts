...
import { useHeroes, useHeroDetail, getHeroQualityColor, HERO_QUALITY_COLORS } from '../hooks/useHero';
...
describe('useHeroes', () => {
  ...
  it('应该加载武将列表', async () => {
    (gameApi.getHeroList as Mock).mockResolvedValue({
      success: true,
      data: mockHeroes,
    });
  ...
  it('应该正确获取武将详情', async () => {
    ...
    (gameApi.getHeroDetail as Mock).mockResolvedValue({
      success: true,
      data: mockHero,
    });
    ...
    expect(gameApi.getHeroDetail).toHaveBeenCalledWith(1);
  });
  ...
  it('应该正确升级武将', async () => {
    ...
    (gameApi.upgradeHero as Mock).mockResolvedValue({
      success: true,
    });
    ...
    expect(gameApi.upgradeHero).toHaveBeenCalledWith(1);
  });
  ...
  it('应该正确解雇武将', async () => {
    ...
    (gameApi.dismissHero as Mock).mockResolvedValue({
      success: true,
    });
    ...
    expect(success).toBe(true);
    expect(gameApi.dismissHero).toHaveBeenCalledWith(1);
  });
});
...
describe('useHeroDetail', () => {
  ...
  it('当 heroId 为 null 时不应该加载', () => {
    ...
    expect(gameApi.getHeroDetail).not.toHaveBeenCalled();
  });
});
