...
import { useCity, useCities } from '../hooks/useCity';
import { cityApi } from '../services/api/cityApi';

// Mock API
vi.mock('../services/api/cityApi', () => ({
  cityApi: {
    getCityInfo: vi.fn(),
...
describe('useCity', () => {
  ...
  it('应该初始加载城市信息', async () => {
    (cityApi.getCityInfo as Mock).mockResolvedValue({
      success: true,
      data: mockCityInfo,
    });

    (cityApi.getBuildingList as Mock).mockResolvedValue({
      success: true,
      data: mockBuildings,
    });
  ...
  it('当 cityId 为 undefined 时不应该加载', () => {
    ...
    expect(cityApi.getCityInfo).not.toHaveBeenCalled();
  });
});
