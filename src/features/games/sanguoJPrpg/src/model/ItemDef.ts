...
module egret {
	...
	export class ItemDef extends egret.DisplayObjectContainer{
        mc:any;
        main:any;
        _state:string;
        cnt:number = 0;
        ...
        public constructor(param1, param2) {
            ...
            var _loc_3:number = GameMain.instance.getMcWidth(this.mc) * GameMain.instance.getMcHeight(this.mc);
            this.price = parseInt(""+_loc_3 / 10) + Math.pow(GameMain.instance.getMcWidth(this.mc) - 30, 2) - (GameMain.instance.getMcHeight(this.mc) - 30) * 10;
            this.cnt = 0;
            this.addP = parseInt(""+_loc_3 / 100) * 0.5;
            ...
            var _loc_4:any = this.mc.x + GameMain.instance.getMcWidth(this.mc) / 2;
            var _loc_5:any = this.mc.y + GameMain.instance.getMcHeight(this.mc) / 2;
            if (_loc_3 > 1000) {
        ...
        onDown(event:egret.TouchEvent) : void {
            ...
            if (this.cnt == 9){
				GameMain.instance.setMessage("LIMIT");
                return;
            }
            if (this.lock_flg)  {
                if (GameMain.instance.key > 0) {
                    ...
                    this.lock_flg = false;
					GameMain.instance.setMessage("I_UNLOCK");
            ...
            if (GameMain.instance.gold >= this.price){
                ...
                this.txt.text = "$" + this.price + "\nx" + this.cnt;
				GameMain.instance.setMessage("BUY_DEF", _loc_2);
				GameMain.instance.disp();
            }else{

            }
				//this.console.log("金币不足  需要："+this.price);
            return;
        }
